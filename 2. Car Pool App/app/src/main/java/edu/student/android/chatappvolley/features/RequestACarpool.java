  ...
  //  ArrayList<Carpool> rcarpool = new ArrayList<>();
    ...
    private boolean checkTolerance(double oslat, double oslng, double odlat, double odlng) {
       List<LatLng> list=  getList(oslat,oslng,odlat,odlng);
        ...
        boolean rsFlag =  edu.student.android.chatappvolley.features.PolyUtil.isLocationOnEdge(rsource,list,true,1000);
        boolean rdFlag =  edu.student.android.chatappvolley.features.PolyUtil.isLocationOnEdge(rdest,list,true,1000);
        if(rsFlag && rdFlag){
            return true;
        }else{
            return false;
